...
const getTripData = time => {
  const res = axios.get(`https://raw.githubusercontent.com/HNU209/Chicago-visualization/main/src/data/trip_data/chicago_trip_${time}.json`);
  const data = res.then(r => r.data);
  return data;
};

const getRestData = type => {
  const res = axios.get(`https://raw.githubusercontent.com/HNU209/Chicago-visualization/main/src/data/rest_data/chicago_${type}.json`);
